...
function AffineCipher(){
    ...
    const encrypt = (plaintext, slope, intercept) => {
        ...
        for(let i = 0; i < plaintext.length;i++){
            const p = plaintext.charCodeAt(i) - 65 //ascii to alphabet
            const c = (slope * p + intercept) % 26 //sisa bagi menjadi alphabet baru
            result += String.fromCharCode(c + 65) //alphabet to ascii
        }
        return result
    }
